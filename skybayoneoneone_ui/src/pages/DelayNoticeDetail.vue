					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateDelayNotice()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2DelayNotices = true">Add</base-button>
								              <modal :show.sync="modal2DelayNotices">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Delay Notice Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="DelayNoticeId" type="text" placeholder="Enter DelayNoticeId" v-model="delayNoticeDetails.delayNoticeId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Delay Time</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="delayNoticeDetails.delayTime"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Reason</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Reason" type="text" placeholder="Enter Reason" v-model="delayNoticeDetails.reason"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      delayNoticeToAdd: {},
      modal2DelayNotices: false,
      delayNoticeDetails: null,
  ...
  methods: {
    ...
    async updateDelayNotice() {

      const jsonData = JSON.stringify(this.delayNoticeDetails);
      const res = await DelayNoticeService.update(jsonData);
...
//        this.getAllDelayNotices();
    ...
    async getDelayNoticeDetails() {
      try {
			let response = await DelayNoticeService.get(this.delayNoticeId);
			this.delayNoticeDetails = response.data;
