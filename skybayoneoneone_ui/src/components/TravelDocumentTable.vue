        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalTravelDocuments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalTravelDocuments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add TravelDocument</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="TravelDocumentId" type="text" placeholder="Enter TravelDocumentId" v-model="travelDocumentToAdd.travelDocumentId"></base-input>
  <base-input label="DocumentType" type="text" placeholder="Enter DocumentType" v-model="travelDocumentToAdd.documentType"></base-input>
  <base-input label="IssueDate" type="text" placeholder="Enter IssueDate" v-model="travelDocumentToAdd.issueDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="travelDocuments" :row-key="record => record.TravelDocumentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <TravelDocumentPictureView :travelDocuments="travelDocuments" />
          </div>
...
import TravelDocumentPictureView from './TravelDocumentPictureView.vue';


const travelDocumentsColumns = [
  "travelDocumentId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    TravelDocumentPictureView
  },

  data() {
    return {
      modalTravelDocuments: false,
        isTableView: true,

      columns: [
        {
          title: 'Travel Document Id',
		dataIndex: 'travelDocumentId',
          visible: true,
          scopedSlots: { customRender: 'travelDocumentId' },
          sorter: true
          //sorter: (a, b) => a.travelDocumentId - b.travelDocumentId,
          //sorter: (a, b) => a.travelDocumentId.localeCompare(b.travelDocumentId),
        },
        {
          title: 'Document Type',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} travelDocuments`,
      },
      ...
      travelDocumentsTable: {
        columns: [...travelDocumentsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'travelDocumentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderTravelDocumentsTable() {
      ...
      let travelDocumentsTableData = [];
      for (let i = 0; i < this.travelDocuments.length; i++) {
        travelDocumentsTableData.push({
          id: i,
          travelDocumentId: this.travelDocuments[i].travelDocumentId,
          year: this.travelDocuments[i].year,
          date: this.travelDocuments[i].date,
          competitionId: this.travelDocuments[i].competitionId,
          travelDocumentId: this.travelDocuments[i].travelDocumentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-travel-documents',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForTravelDocumentsChanged', this.searchQuery);
		//this.renderTravelDocumentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalTravelDocuments = false;

      const currentDate = new Date().getTime();
      this.travelDocumentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.travelDocumentToAdd);
      console.log(jsonData);
      
      const res = await TravelDocumentService.addTravelDocument(jsonData);
